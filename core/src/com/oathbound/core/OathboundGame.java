    ...
    private void renderGameplay() {
        ...
        if (boss != null) {
            if (!boss.isDead()) {
                ...
                if (player.getAttackHitbox().width > 0 && player.getAttackHitbox().overlaps(boss.getBounds())) {
        ...
        for (Enemy enemy : enemies) {
            ...
            if (enemy.getAttackHitbox() != null && enemy.getAttackHitbox().width > 0) {
                if (playerDamageTimer <= 0 && player.getBounds().overlaps(enemy.getAttackHitbox())) {
                    player.takeDamage(1);
                    playerDamageTimer = 1.0f;
                }
            }
        ...
        for (Projectile p : projectiles) {
            ...
            if (p.isAOE()) {
                ...
                if (!p.isExploding()) {
                    for (Enemy enemy : enemies) {
                        if (enemy.isActive() && p.getBounds().overlaps(enemy.getBounds())) {
                            hitEnemyDirectly = true;
                            break;
                        }
                    }
                    if (boss != null && !boss.isDead() && p.getBounds().overlaps(boss.getBounds())) {
                        hitEnemyDirectly = true;
                    }
                }
